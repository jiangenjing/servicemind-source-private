...
评测除了检查工具是否返回结果，还要说明该结果是否解决了用户的问题。建议先用同一组标注请求比较“单 Agent 基线”和“当前多角色编排”，固定模型、知识库版本与预算；多角色只有在复合诉求覆盖或可控性上带来价值，才值得额外延迟和费用。

| 产品问题 | 工程观察点 | 建议记录的结果 |
| --- | --- | --- |
| 用户是否需要反复解释 | 同会话记忆是否保留已确认字段 | 重复询问次数、完成任务所需轮数 |
| 回答能否让用户继续操作 | 排障步骤、缺失字段和下一步是否明确 | 人工标注的可行动性与任务完成率 |
| 知识是否真的支持答案 | 检索片段与最终回答的对应关系 | 有依据的关键结论比例、错误引用案例 |
| 是否正确处理能力边界 | 未接入订单、退款、工单时的回复 | 越权成功声明数量，应优先消除 |
| 多角色是否值得 | 与单角色基线比较覆盖、延迟、费用 | 每个有效完成任务的成本，而非单次调用价格 |

任务完成需要预先定义。例如，技术咨询可定义为提供符合演示知识的排查步骤；账单核验可定义为收集必要字段并正确计算差值，不能定义成当前系统无法执行的“退款成功”。评测记录建议包含 case_id、期望行为、实际路由、工具证据、最终回复、人工判断、延迟和模型费用；不将用户原始敏感信息直接写入公开报告。

以上为下一轮实验协议，当前没有填入未经运行验证的提升率。先建立小而可核验的用例集，再扩大样本，比先接更多 Agent 更容易识别系统的真实收益。
